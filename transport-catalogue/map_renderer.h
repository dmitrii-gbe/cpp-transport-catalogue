...
MapRenderer(RenderingSettings& settings) : settings_(settings) 
{
}

std::map<std::string_view, std::tuple<std::vector<std::pair<std::string, svg::Point>>, bool>> TranformCoordinates(const std::unordered_map<std::string_view, transport_catalogue::Bus*>& routes) const;

svg::Document RenderRoutes(const std::map<std::string_view, std::tuple<std::vector<std::pair<std::string, svg::Point>>, bool>>& routes) const;
